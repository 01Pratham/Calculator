...
function elem()
{
    ...
    ?>
    <div class="form-group col-md-3 mx-2" id="<?= $prod . "_{$id}" ?>">
        <span class="fa fa-remove text-danger" style="position: absolute; margin: 4px 0px 0px -13px; cursor: pointer;"
        ...
        <div class="row">
            ...
            <input type='number' aria-describedby="<?= "{$prod}unit_{$id}" ?>" class='form-control small col-md-8'
                id='<?= '{$category}_qty_{$id}' ?>' <?= (!$input_box) ? "disabled" : '' ?> min=0 placeholder='Quantity'
                name='<?= "{$category}_qty[{$name}][]" ?>'>
            <span class="input-group-text form-control col-4 bg-light p-0" id="<?= "{$prod}unit_{$id}" ?>">
                <?php
