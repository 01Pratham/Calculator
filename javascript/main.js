...
function add_vm(count = null, name, id, cloneId = '', lastVM = false) {
    ...
    // console.log(count_of_virtual_machine);
...
function cloneEst($this) {
    ...
    var count_id = count_of_est + "" + 1;

    let section = $parent.clone();
    ...
    section.find("*").each(function () {
        ...
        if ($(this).data("id") !== "") {
            $(this).data("id", newID.join("").replace(/_/g, ""))
        }

        if ($(this).prop("id") == "add-estmt") {
            $(this).attr("id", `rem-estmt_${count_id}`).val("×").addClass("rem-est").click(function () { $(this).parent().parent().remove() })
        }
        ...
        if ($(this).attr("aria-controls")) {
            ...
            $(this).attr("aria-controls", control.join("_")).attr("href", `#${control.join("_")}`)
        }
    });

    // console.log(id)
    // let sect = `<section class="est_div align-center Main mt-2" id="est_div_${id}" style="background:#f4f6f9; "></section>`

    // $('#myTab').append(`<section class="est_div align-center Main mt-2" id="est_div_${count_of_est}" style="background:#f4f6f9; "></section>`);
    $(`#myTab`).append(section)
}



function addLineItemsToDropdownMenu() {
    $(".dropdown-menu").html("");
    $(".EstmtName").each(function () {
        let $name = ($(this).val() === '') ? "Your Estimate" : $(this).val();
        $(".dropdown-menu").append(`<div class="dropdown-item" style = "cursor: pointer;" data-id = "${$(this).data("id")}" data-name = "${$(this).data("name")}" onclick = "addProductAjax($(this))" >${$name}</div>`);
    })
}

function addProductAjax($this) {
...
function makeValuesUnique(obj) {
    ...
    return Object.fromEntries(Object.keys(obj).map((key, index) => [key, updatedValues[index]]));
}
